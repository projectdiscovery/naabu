...
SERVICES-DISCOVERY:
   ...
   -sV-probes string                 custom nmap-service-probes file path (uses managed cache or local nmap install if empty)
   -sV-update-probes                 update managed nmap-service-probes cache and exit
...
| `-sV-workers int` | Number of concurrent service version workers (default 25) |
| `-sV-probes string` | Custom nmap-service-probes file path (uses managed cache or local nmap install if empty) |
| `-sV-update-probes` | Update the managed nmap-service-probes cache and exit |
| `-sD` | Service discovery (match port number to service name, no active probing) |

The `-sV` flag requires the `nmap-service-probes` database. naabu does not ship this database itself, as it is licensed under the copyleft Nmap Public Source License which is incompatible with naabu's MIT license. naabu resolves the database in this order: a custom file passed via `-sV-probes`, then the managed user cache, then a locally installed nmap copy auto-detected from standard installation paths. To populate or refresh the managed cache from the official Nmap source, run `naabu -sV-update-probes`.
...
- `-uP` reuses the same probe database as `-sV`, so you can combine the two without paying the parse cost twice. The probe database is resolved the same way as for `-sV` (custom `-sV-probes` file, then managed cache, then local nmap install). If no database can be found `-uP` logs a warning and is silently disabled.
